...
export function MissionPage() {
    ...
    const currentChallenge = algebraMissions.find((mission) => mission.id === currentId) ?? null
    const filtered = selectedDifficulty ? algebraMissions.filter((mission) => mission.difficulty === selectedDifficulty) : []
    const totalPoints = completed.reduce((sum, id) => sum + (algebraMissions.find((mission) => mission.id === id)?.points ?? 0), 0)
            ...
            <div className="mission-page">
                <div className="container">
                    <header className="mission-header">
                        <h1>🎯 {pickText(lang, { eu: 'Aljebraren Erronkak', es: 'Retos de Álgebra', ar: 'تحديات الجبر' })}</h1>
                        <p className="mission-subtitle">{pickText(lang, { eu: 'Ariketa gidatuak hiru mailatan, teoria eta laborategia sendotzeko.', es: 'Ejercicios guiados en tres niveles para consolidar teoría y laboratorio.', ar: 'تمارين موجهة في ثلاثة مستويات لترسيخ النظرية والمختبر.' })}</p>
                    </header>
                    ...
                    <div className="difficulty-grid">
                        {(Object.keys(difficultyMeta) as Difficulty[]).map((difficulty) => {
                            const missions = algebraMissions.filter((mission) => mission.difficulty === difficulty)
    ...
    const check = () => {
        ...
        if (currentChallenge.answer.some((candidate) => candidate.replace(/\s+/g, '').toLowerCase() === normalized)) {
            setFeedback('success')
